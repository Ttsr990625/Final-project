...
void RtxApp::Update(const size_t, const float dt) {
    // Update FPS text
    String frameStats = ToString(mFPSMeter.GetFPS(), 1) + " FPS ";
...
void RtxApp::LoadSceneGeometry() {
  ...
  //  String fileName = sScenesFolder + "fake_whitted/test.obj";
   String fileName = sScenesFolder + "fake_whitted/test2.obj";
    String baseDir = fileName;
    const size_t slash = baseDir.find_last_of('/');
    if (slash != String::npos) {
        baseDir.erase(slash);
    }
   // String fileName1 = sScenesFolder + "fake_whitted/Medieval_building.obj";
   // String baseDir1 = fileName1;
   // const size_t slash1 = baseDir1.find_last_of('/');
   // if (slash1 != String::npos) {
   //     baseDir1.erase(slash1);
   // }
    const bool result = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &error, fileName.c_str(), baseDir.c_str(), true);
    //const bool result1 = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &error, fileName1.c_str(), baseDir1.c_str(), true);
    if (result) {
...
void RtxApp::CreateScene() {
   ...
   // mEnvTexture.Load((sEnvsFolder + "white.jpg").c_str());
   mEnvTexture.Load((sEnvsFolder + "lago_disola_2k.jpg").c_str());
